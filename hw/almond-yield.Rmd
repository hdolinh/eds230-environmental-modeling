...
- tmax_c is year?
- we're only using tmin? If we're looking at the formula? I don't see Tx (max temp)?

```{r}
almond_yield <- function(temp_max, temp_min, precip){
  
  ((-0.015 * temp_max) - (-0.0046 * temp_min^2)) - ((0.07 * precip) + (0.0043 * precip^2)) + 0.28
}
```
...
```{r}
# 2000 test
clim_data_2000 <- clim_data %>% 
  filter(year == "2000") %>% 
  mutate(mean_tmax_c = mean(tmax_c)) %>% 
  mutate(mean_tmin_c = mean(tmin_c)) %>% 
  mutate(mean_precip_mm = mean(precip))

ay_2000 <- almond_yield(temp_max = 21.61513, 
                        temp_min = 12.18142,
                        precip = 1.702128)
```
